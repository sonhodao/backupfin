...
@section('content')
    <!-- Start of Main -->
        {{ Breadcrumbs::render('fe.update.pass')}}
                    ...
                    <div class="tab tab-nav-boxed tab-nav-center tab-nav-underline">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active">
                                <form action="{{ route('fe.account.reset') }}" method="POST">
                                    @csrf
                                    <div class="form-group">
                                        <label class="fs16">Tài khoản: <b>{{$email}}</b> </label>

                                    </div>
                                    <div class="form-group input-login-register">
                                        <input type="hidden" name="email" value="{{$email}}">
                                        <input type="hidden" name="token" value="{{$token}}">
                                        <input type="password" class="form-control fs16" name="password" placeholder="Nhập mật khẩu mới" required   id="ipnPasswordReset">
                                        <div class="eye-password" id="btnPasswordReset">
...
@push('scripts')
    <script>
        // step 1
        const ipnElement = document.querySelector('#ipnPasswordReset')
        const btnElement = document.querySelector('#btnPasswordReset')
